...
#let question(x) = text(fill: green, x)
#let answer(x) = text(fill: red, x)
...
#answer("Die Antwort auf die Frage aller Fragen ist 42.\n")
...
- Einfach gestaltete GUI-Skizzen (Mockups) von mindestens zwei wesentlichen 
GUI-Komponenten (Hauptseite, Tabs, etc.). Die Skizzen können mit einem einfachen 
Grafikprogramm erstellt werden. Auch sorgfältige Handzeichnungen sind erlaubt. 
Keine Login-GUI skizzieren!
...
- Entwurfsklassendiagramm (Untersuchen Sie dabei den Einsatz geeigneter 
Entwurfsmuster)
- GUI-Modellierung:
